class Param {
  constructor (func, name, defaultValue, parent = '', types=[], description = '') {
    this.types = types
    this.name = name
    this.parent = parent
  ...
  static parse (func, string) {
    ...
    const types = match[1].trim().slice(1, -1).split('|').filter(type => type)
    return new Param(func, match[2], undefined, undefined, types, match[3])
  }
