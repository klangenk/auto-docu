class Param {
  constructor (name, defaultValue, parent = '', types=[], description = '') {
    this.name = name
    this.fullName = parent ? `${parent}.${name}` : name
    this.optional = !!defaultValue
    this.defaultValue = defaultValue
    this.types = types
    this.description =description
    this.props = {}
    this.calls = 0
  }

  static parse (string) {
    const regex = /@\S+ ((?:\{.*\})?) (\S+)(.*)/
    const match = string.match(regex)
    const types = match[1].slice(1, -1).split('|').filter(type => type)
    console.log(types)
    return new Param(match[2], undefined, undefined, types, match[3])
  }

  addProp (key, value, parent = this.fullName) {
    if(!this.props[key]) {
      this.props[key] = new Param(key, undefined, parent)
    }
    this.props[key].addExample(value)
  }

  addExample (example) {
    this.calls++
    if (example === null || example === undefined) {
      this.optional = true
      return
    }

    const type = getType(example)

    if (type === 'Object') {
      Object.keys(example)
        .filter(key => !key.startsWith('_'))
        .forEach(key => this.addProp(key, example[key]))
    }

    if (type.includes('[]')) {
      if (type.startsWith('Object')) {
        const depth = (type.match(/\[/g) || []).length
        let obj = example
        for (let i = 0; i < depth; i++) {
          obj = obj[0]
        }
        const parent = `${this.fullName}${'[]'.repeat(depth)}`
        Object.keys(obj)
          .filter(key => !key.startsWith('_'))
          .forEach(key => this.addProp(key, obj[key], parent))
      }

    }

    if (this.types.includes(type)) return
    this.types.push(type)
  }

  addExamples (examples) {
    examples.forEach((example) => this.addExample(example))
  }

  checkOptional () {
    Object.values(this.props).forEach(prop => {
      if (prop.calls < this.calls || this.optional) {
        prop.optional = true
      }
    })
  }

  toString () {
    this.checkOptional()
    const types = this.types.join('|')
    let name = this.fullName
    if (this.defaultValue !== undefined) name = `${name}=${this.defaultValue.replace(/'|"/g,'')}`
    if (this.optional) name = `[${name}]`
    const param =`@param {${types}} ${name} ${this.description.trim()}`.trim()
    let doc = [` * ${param}`]

    if (Object.keys(this.props).length <= 10) {
      doc = doc.concat(Object.values(this.props).map(prop => prop.toString()))
    }
    return doc
      .join('\n')
  }
}


function getType(example) {
  if (example === '__FUNCTION__') return 'function'
  let type = typeof example
  if (type === 'object') {
    if (Array.isArray(example)) {
      if (example.length) {
        return `${getType(example[0])}[]`
      }
      return 'Array'
    }
    return 'Object'
  }
  
  return type
}

module.exports = Param