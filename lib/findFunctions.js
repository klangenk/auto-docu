...
function findFunctions (source) {
  ...
  let regex = new RegExp(`${comment}${indentation}[^,\n\(]*?${fName}\\s*?${paramList}\\s*?\\{`, 'gm')
  ...
  regex = new RegExp(`${comment}${indentation}.*?${fName}\\s*?=\\s*${paramListArrow}\\s*?=>\\s*(\\{?)`, 'gm')
  ...
  while (match = regex.exec(source)) {
    const func = {
      ...
      body: {
        start: match.index + match[0].length,
        end: match[5] ? findEnd(source, match.index + match[0].length) : match.index + match[0].length
      }
    }
    functions.push(func)
  }
